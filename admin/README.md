...
|-----|---------------|-----------|
| **Members** | All `member:{email}` rows | Photo column (thumbnail of a live profile photo, or a *flagged* marker), **flag photo** / **unflag photo** (proxies the join Worker's `POST /admin/member/avatar/{flag,unflag}` — after-the-fact moderation: deletes the photo, drops it from `members:all`, shows the member your reason on `/edit` and blocks that exact image; see `docs/features/member-profile.md` § Moderation), clear rate limits, force-unlink Letterboxd (proxies the join Worker's `POST /admin/member/unlink` — full cascade incl. `members:all` + `update-member` dispatch; shows as "repair LB" when only the aggregate still carries a handle), evict session snapshot |
| **Newsletter** | Compose/send a newsletter, opted-in recipients (with per-member opt-in/out toggles), and send history (`newsletter:sent:{ts}`, each row carrying a **view shared copy** link to its `/n/{id}` archive — rows written before the archive existed have no `archiveId` and show a dash) | toggle a member's `newsletter` flag (evicts their session), insert an "Upcoming events" section (all events with `date` >= Central today from `event:` KV rows, soonest first; the formatter whitelists public-projection fields only — a house host's private address/notes never reach the newsletter), insert a "Latest from members on Letterboxd" section (live diary entries via `GET /api/watched`, count configurable, appended to both HTML and text bodies, labeled "Entries"), insert a **flyer** (paste or pick an image and it is staged — decoded, downscaled to fit the card, re-encoded as JPEG and uploaded to R2 on insert, returning a public `join.jxnfilm.club/nl/img/{sha256}.jpg` URL — or paste a URL you host yourself: image + required alt text + required details, gated by `imageBlockIssues` — the details render as real text so the date/time/RSVP survive image blocking and screen readers; pasting an image into the preview instead embeds a data: URI, which Gmail strips at render time, so embedded images are scrubbed out of the preview on sight), insert a linked TMDB poster (search via `GET /api/tmdb/search`, pick a thumbnail, optional link URL wraps the poster; the block assumes a review or announcement follows — centered poster, then a left-aligned headline + bracketed placeholder body to overwrite in the preview; sending is blocked while the placeholder text remains), undo the last insert (button or ⌘Z; confirms if you've edited since), send a test to one address, send to all opted-in members. Every send (test included) also publishes the newsletter at a shareable permalink (`join.jxnfilm.club/n/{id}`), linked from the footer as "view it in your browser" — the escape hatch for forwarding, Gmail clipping, blocked images and mangled layouts. The archive stores the PRE-footer body: a rendered message carries a per-recipient signed unsubscribe token, and publishing one would let anyone unsubscribe that member. Served with a script-blocking CSP, since the body is operator-authored HTML on our own origin |

A running byte budget sits under the compose fields. Sends and **test sends** are blocked on identical terms by `newsletterSendBlocker`: an embedded `data:` image (Gmail strips those at render time, so a test to an Apple Mail address looks perfect while the list sees nothing), a cross-environment `/nl/img/` URL, a body past ~92KB (Gmail clips near 102KB), or the leftover poster placeholder. The join Worker enforces its own looser ceiling (413/400) *before* the `testTo` branch, and surfaces Resend failures as 502 with the upstream text plus a partial-send count rather than "internal server error".
...
| **Content Gen → Event poster (no film)** | A 2:3 title card drawn from the event's own title — the artwork a social event has no film poster to supply. **Use as event poster** uploads the PNG to the newsletter image store and sets it as the event's `poster`, so the newsletter table and social cards pick it up too. The public site renders an equivalent card in CSS for every poster-less event anyway, so this is for the surfaces that need a real image. |
| **Events** | `event:{id}` rows + `events:all` aggregate from `ATTENDANCE_KV`, live attendance from `attend:{id}`; sortable (upcoming first / newest / oldest / title / most attended) | create events through a validated form (title + date required, id derived from date + film, TMDB film search) — nothing is written until it passes, so no `Untitled` placeholder is ever public; edit / delete — all proxied through the join Worker's `PUT`/`DELETE /admin/events/:id` (never raw KV writes: a `/api/kv` PUT cannot promote a waitlist, enforce the capacity guard, or email the RSVP list when you postpone something). `GET /events` surfaces the change immediately on the public site. A save on an event with RSVPs offers a **notify** checkbox naming its audience (off by default, confirm dialog before sending); a delete always sends cancellation notices. Also: remove attendees (by row, not by name — two attendees can share one) |
...
| **Voice prompt** | `config:voice_prompt` | `{ id, text, deadline? }` — the question members answer when recording a podcast voice clip. The id is a slug that keys the submissions (`voice:{id}:…`), so changing it starts a fresh collection; the optional deadline (YYYY-MM-DD) is display-only. An id beginning `msg_` is refused — that namespace belongs to members' free-form messages, and a round pointed at it would collide with their rows. Reset deletes the key and the site falls back to the generic default prompt (id `general`, "Tell us what you're watching"). |
...
`--clips-only` is not optional here. Without it the script also renders a
one-clip "segment" credited to *"1 member · JXN Film Club"*, throwing away the
speaker credit — the whole point of the format. `compile_voices.mjs msg_…` also
works and produces a one-clip segment.
