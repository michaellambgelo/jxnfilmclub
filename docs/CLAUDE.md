...
- **KV schema**:
  ...
  - `avatars:cache` — `{ sig, map }`: handle-keyed Letterboxd avatar URLs scraped from the `/{handle}/films/` subpage header (`parseProfileAvatar`; the profile ROOT page is behind Letterboxd's Cloudflare bot challenge since ~Aug 2026, and the subpage og:image is a generic share card). Size segment rewritten to 80px. 7-day TTL; a TOTAL miss is negative-cached for 1h so page loads don't re-fire the fan-out while Letterboxd is unreachable. `sig` is the sorted linked-handle list, recomputed on every `GET /avatars` hit — a handle set change busts either cache immediately. Members without a custom upload (`/static/img/` defaults) never match the parser, so they keep the letter avatar.
