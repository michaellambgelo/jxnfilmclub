...
| `tests/worker/` | Vitest + Workers pool — 17 endpoint/behavior suites: signup, otp, letterboxd (self-service + admin unlink), member-update, member-delete, members-events, members-reconcile, newsletter, scrub, screenings, attendance, watched, avatars, security, session-refresh, tmdb, pages |
...
- **KV schema**:
  ...
  - `watched:cache` — handle-keyed map of last-four diary entries from Letterboxd RSS, 900s TTL. Written by `GET /watched`'s fan-out; deleted by the unlink cascade so an unlinked member's RSS stops serving immediately.
  - `avatars:cache` — `{ sig, map }`: handle-keyed Letterboxd avatar URLs scraped from profile `og:image`, 7-day TTL. `sig` is the sorted linked-handle list, recomputed on every `GET /avatars` hit — a handle set change rebuilds immediately instead of waiting out the TTL. Default avatars (`/static/img/` assets) are filtered so those members keep the letter avatar.
